...
request
	-> consoleRequestLoggerMiddleware() or databaseRequestLoggerMiddleware()
		 ...
		 -> extract and normalize request data (DEFAULT fallbacks)
...
| `__tests__/request-logger.test.ts` | Module tests                                       |

## Key Responsibilities

- Extract and normalize request data before persistence.
- Use shared `applyRequestDefaults` for override + `DEFAULT` fallback normalization before persistence.
...
- Tests: [tests/request-logger.test.ts (**tests**)](__tests__/request-logger.test.ts)
